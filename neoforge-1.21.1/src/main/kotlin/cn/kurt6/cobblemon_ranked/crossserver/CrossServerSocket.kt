...
object CrossServerSocket {
    ...
    private val playerMap = ConcurrentHashMap<String, ServerPlayer>()
    ...
    private var connectionInitiator: CommandSourceStack? = null

    private fun executeOnPlayerServer(player: ServerPlayer, action: () -> Unit) {
        player.server.execute(action)
    }

    private fun executeOnSourceServer(source: CommandSourceStack, action: () -> Unit) {
    ...
    fun connect(source: CommandSourceStack? = null) {
        ...
        heartbeatJob = CoroutineScope(IO).launch {
            ...
            connectionInitiator?.let { source ->
                ...
                executeOnSourceServer(source) {
                    source.sendMessage(Component.literal(successMessage))
                }
            }
            while (isActive) {
                delay(20_000)
...
//        log("cross.log.request_battle_state", "battleId" to battleId)
    ...
    private fun handleBattleUpdate(json: JsonObject) {
        ...
        executeOnPlayerServer(player) {
            ...
            player.sendMessage(Component.literal(MessageConfig.get("cross.battle.current_pp", lang)))
            ...
            selfMoves?.forEachIndexed { index, moveElement ->
                ...
                player.sendMessage(Component.empty()
                    ...
                    .append(Component.literal(" $name §7(PP: $currentPP/$maxPP)")))
            }
    ...
    fun joinMatchmakingQueue(player: ServerPlayer, pokemonList: List<JsonObject>, mode: String) {
        val lang = config.defaultLang
    ...
    fun leaveMatchmakingQueue(player: ServerPlayer) {
        val lang = config.defaultLang
        if (!config.enableCrossServer) {
            RankUtils.sendMessage(player, MessageConfig.get("cross.cross_server_disabled", lang))
            return
    ...
    fun sendBattleCommand(player: ServerPlayer, commandType: String, data: String) {
        val lang = config.defaultLang
...
//            "command" to commandStr)
    ...
    private fun showCurrentPokemonMoves(player: ServerPlayer, session: BattleSession) {
        val lang = config.defaultLang
        ...
        // 发送标题
        player.sendMessage(Component.literal(MessageConfig.get("cross.battle.current_moves", lang)))
        ...
        for (i in 0 until minOf(4, initialMoves.size())) {
            ...
            player.sendMessage(Component.empty()
                ...
                .append(Component.literal(" $moveName §7(PP: $currentPP/$maxPP)"))
            )
        }

        if (displayedCount == 0) {
            player.sendMessage(Component.literal(MessageConfig.get("cross.battle.no_moves_available", lang)))
        }

        player.sendMessage(Component.literal(MessageConfig.get("cross.battle.click_hint", lang)))
    }

    private fun showSwitchOptions(player: ServerPlayer, session: BattleSession) {
        val lang = config.defaultLang
        ...
        team.forEachIndexed { index, pokemon ->
            ...
            // 创建可点击的切换链接
            val switchText = Component.empty()
                .append(link("[${index + 1}]", "/rank cross battle switch ${index + 1}"))
                .append(Component.literal(" $name §7(HP: $hpPercent%)"))

            player.sendMessage(switchText)
        }
    }

    private fun link(text: String, command: String, hoverKey: String? = null): Component {
        val lang = CobblemonRanked.config.defaultLang
    ...
    fun sendChatMessage(player: ServerPlayer, message: String) {
        val lang = config.defaultLang
        val session = battleSessions[player.uuidAsString] ?: run {
            RankUtils.sendMessage(player, MessageConfig.get("cross.battle.no_active", lang))
            return
    ...
    private fun sendForfeitCommand(player: ServerPlayer) {
