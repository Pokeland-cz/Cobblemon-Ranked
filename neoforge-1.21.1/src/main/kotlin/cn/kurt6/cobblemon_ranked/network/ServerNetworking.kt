...
class ServerNetworking {
    companion object {
        ...
        fun handle(payload: RequestPlayerRankPayload, context: IPayloadContext) {
            val player = NeoForgeNetworking.player(context)
        ...
        private fun handlePlayerRequest(player: ServerPlayer, format: String) {
            ...
            if (data != null) {
                ...
                NeoForgeNetworking.sendToPlayer(player, response)
            } else {
                RankUtils.sendMessage(player, MessageConfig.get("rank.not_found", lang))
            }
        }

        private fun handleSeasonRequest(player: ServerPlayer, format: String) {
            ...
            NeoForgeNetworking.sendToPlayer(player, SeasonInfoTextPayload(message.replace("\\n", "\n")))
        }

        private fun handleLeaderboardRequest(player: ServerPlayer, format: String, pageStr: String?) {
            ...
            NeoForgeNetworking.sendToPlayer(player, LeaderboardPayload(leaderboardText, page))
        }
    }
}
