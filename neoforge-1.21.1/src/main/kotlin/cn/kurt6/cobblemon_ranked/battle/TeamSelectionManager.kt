...
object TeamSelectionManager {
    ...
    fun handleDisconnect(player: ServerPlayer) {
        ...
        synchronized(session) {
            ...
            if (NeoForgeNetworking.canSend(opponent, TeamSelectionEndPayload.TYPE)) {
                NeoForgeNetworking.sendToPlayer(opponent, TeamSelectionEndPayload.INSTANCE)
            }
    ...
    private fun sendStartPacket(
        ...
        NeoForgeNetworking.sendToPlayer(target, payload)
    }

    fun handleSubmission(player: ServerPlayer, selectedUuids: List<UUID>) {
        val session = pendingSessions[player.uuid] ?: return
    ...
    private fun validateSubmission(player: ServerPlayer, selected: List<UUID>, limit: Int, format: BattleFormat): Boolean {
    ...
    private fun teleportAndStartBattle(
        ...
        if (NeoForgeNetworking.canSend(session.player1, TeamSelectionEndPayload.TYPE)) {
            NeoForgeNetworking.sendToPlayer(session.player1, TeamSelectionEndPayload.INSTANCE)
        }
        if (NeoForgeNetworking.canSend(session.player2, TeamSelectionEndPayload.TYPE)) {
            NeoForgeNetworking.sendToPlayer(session.player2, TeamSelectionEndPayload.INSTANCE)
        }
    ...
    private fun startActualBattle(
        p1: ServerPlayer, t1Uuids: List<UUID>,
        p2: ServerPlayer, t2Uuids: List<UUID>,
    ...
    private fun getBattlePokemon(player: ServerPlayer, uuid: UUID): BattlePokemon? {
        val party = Cobblemon.storage.getParty(player)
