...
import cn.kurt6.cobblemon_ranked.network.NeoForgeNetworking
...
class MatchmakingQueue {
    ...
    companion object {
        fun handlePlayerDisconnect(player: ServerPlayer) {
            ...
            try {
                val activeBattle = Cobblemon.battleRegistry.getBattleByParticipatingPlayer(player) != null
                val inSelection = TeamSelectionManager.isPlayerInSelection(uuid)
                if (!activeBattle && !inSelection) {
    ...
    fun addPlayer(player: ServerPlayer, formatName: String) {
        ...
        if (config.enableTeamPreview && !NeoForgeNetworking.canSend(player, TeamSelectionStartPayload.TYPE)) {
        ...
        if (formatName.lowercase() == "2v2singles") {
            try {
                ...
                player.server.playerManager.broadcast(net.minecraft.network.chat.Component.literal(MessageConfig.get("queue.global_join", lang, "player" to player.name.string, "format" to formatName)), false)
            } catch (e: Exception) {
    ...
    fun getPlayer(playerId: UUID, format: String? = null): ServerPlayer? {
    ...
    private fun startRankedBattle(player1: QueueEntry, player2: QueueEntry) {
        ...
        BattleHandler.requestArena(
            ...
            onArenaFound = { arena, positions ->
                val worldId = ResourceLocation.tryParse(arena.world)
                val worldKey = if (worldId != null) ResourceKey.create(Registries.DIMENSION, worldId) else null
                val world = if (worldKey != null) server.getLevel(worldKey) else null
    ...
    private fun getPlayerTeam(player: ServerPlayer): List<UUID> {
    ...
    data class QueueEntry(val player: ServerPlayer, val format: BattleFormat, val team: List<UUID>, val joinTime: Long)
    ...
    fun canPlayerJoinQueue(player: ServerPlayer): Boolean {
